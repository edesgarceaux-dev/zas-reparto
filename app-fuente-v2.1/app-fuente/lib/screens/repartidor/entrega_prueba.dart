  ...
  Future<void> _finalizar() async {
    ...
    try {
      ...
      for (final slot in _fotos.keys) {
        final archivo = _fotos[slot]!;
        final ruta =
            '$id/${slot}_${DateTime.now().millisecondsSinceEpoch}.jpg';
        final bytes = await File(archivo.path).readAsBytes();
        await supa.storage.from('entregas').uploadBinary(ruta, bytes,
            fileOptions:
                const FileOptions(contentType: 'image/jpeg', upsert: true));
        urls[slot] = supa.storage.from('entregas').getPublicUrl(ruta);
      }
      await supa.from('entregas_prueba').insert({
        'pedido_id': id,
        'nombre_recibe': _nombre.text.trim(),
        'rut_recibe': _rut.text.trim().toUpperCase(),
      ...
      await supa.from('pedidos').update({
        'estado': 'entregado',
        'nota_entrega':
            'Recibió: ${_nombre.text.trim()} (${_rut.text.trim().toUpperCase()})',
      }).eq('id', id);
      if (mounted) {
      ...
      setState(() => _error =
          'No se pudo guardar. Revisa tu internet e intenta de nuevo.');
    } finally {
